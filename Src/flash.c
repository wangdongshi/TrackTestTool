...
int eraseSector(const unsigned short sectorNo)
{
  ...
  if (HAL_FLASHEx_Erase(&flashSet, &sectorError) != HAL_OK) return 0;
  if (FLASH_WaitForLastOperation(1000) != HAL_OK) return 0;
                     ...
                     const unsigned short size)
{
  ...
  for (unsigned short i = 0; i < size; i += 2) {
    ...
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, address + i, tmpbuf) != HAL_OK) return 0;
  }
                        ...
                        const unsigned short size)
{
  ...
  if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE, address + size,     (unsigned char)(crc & 0x00FF))        != HAL_OK) return 0;
  if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE, address + size + 1, (unsigned char)((crc & 0xFF00) >> 8)) != HAL_OK) return 0;
